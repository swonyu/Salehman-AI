...
struct KnowledgeRAGTests {
    ...
    func chunkEmptyOrShortReturnsAsExpected() {
        ...
        #expect(KnowledgeStore.chunk("   ") == [])
        #expect(KnowledgeStore.chunk("short") == ["short"])
        // Non-ws token > size still produces 1 (or 2) chunk(s) — impl backs up only on ws; >size/2 guard not met so may emit full-size slices.
        let big = KnowledgeStore.chunk("1234567890", size: 5)
        #expect(big.count >= 1 && big.count <= 3)
        #expect(big.joined().contains("12345"))
    }
