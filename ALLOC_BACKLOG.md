...
### ✅ ALREADY DONE (superseded, superior implementation) #1 — StockSageCapitalAllocator.allocate — fractional-Kelly multi-idea allocator, heat-capped  [medium]
**signature:** ```swift
...
### ✅ DONE #2 — StockSageCapitalAllocator.suggestAdd — marginal allocation against the live book  [medium]
**signature:** ```swift
...
### ✅ ALREADY DONE (merged into #1's implementation) #3 — StockSageCapitalAllocator.allocateDeCorrelated — cluster-merged heat-capped allocator  [large]
**signature:** ```swift
enum StockSageCapitalAllocator {
...
### ✅ SUBSTANTIALLY DONE (engines already exist + wired) #4 — StockSageCapitalAllocator.rebalanceByRiskParity — whole-book equal-risk reweight  [medium]
**signature:** ```swift
enum StockSageCapitalAllocator {
...
**2026-07-01 adversarial-review fixes (2 confirmed moderate gaps):** (1) the correlation gate only checked new-vs-HELD, never new-vs-new — a mutually-correlated clique of brand-new ideas (e.g. two correlated crypto names into a near-empty book) could slip through completely unchecked. Fixed: a clique within the new-idea pool itself is now de-weighted via `StockSageCorrelationCluster.correlationAdjustedWeights`, mirroring `allocate`'s own Step 2.5. (2) unlike every other sizing tool in this engine, there was no per-position concentration ceiling — a sole (or dominant) positive-EV name could be recommended at up to 100% of the book, which 3 of the shipped tests demonstrated as "correct" behavior. Fixed: targets are now capped at `StockSageKelly.maxFraction` when enough funded names exist to redistribute the excess under it; when they don't (capping would be silently UNDONE by `Rebalance.plan`'s own renormalization — dividing a lone value by itself is always 1.0), the plan is left at its honest concentrated weights with a loud "CONCENTRATION WARNING" note instead of a false sense of safety. 3 new regression tests.

### ✅ DONE (re-spec'd, Frank-Wolfe not projected-gradient) #6 — StockSageAllocationOptimizer.optimizeSharpeDeCorrelated — Sharpe-max QP allocator (stretch)  [large]
**signature:** ```swift
...
**2026-07-01 implementation (owner "do everything," after the 2026-07-01 re-audit found the literal spec unbuildable):** Re-spec'd `ideas` from the doc's raw tuple (which had no volatility input for Sigma — the re-audit's blocking finding) to `[StockSageIdea]`, deriving each idea's R-multiple volatility from its own `spark` (`annualizedVolatility(spark) / stopDistancePct` — a tighter stop makes a given price move a LARGER number of R's) and correlations from `StockSagePortfolioAnalytics.correlationMatrix` over `dailyReturns(spark)` pairs — no new fetch. Added the missing `lambda` (risk-aversion) parameter the re-audit also flagged, defaulted to **1.0 with a real derivation, not an invented number**: for a single asset this objective's first-order condition reduces to `w* = mu/(2·lambda·sigma²)`, and `lambda=1` reproduces EXACTLY today's HALF-Kelly fraction as the single-asset special case — anchoring the multi-asset optimizer's default risk tolerance to the same appetite `StockSageKelly.maxFraction` already ships with. Solver: **Frank-Wolfe (conditional gradient), NOT projected gradient** — prototyped both in Python before writing any Swift; projected gradient with a textbook-correct Lipschitz step reliably collapsed to a trivial equal-split answer under this problem's tight-heat-cap-vs-large-unconstrained-optimum geometry (a single "correct" step blows every coordinate past its box bound simultaneously, and the projection's water-filling then returns a spurious, falsely-"converged" symmetric fixed point regardless of the true mu/Sigma) — Frank-Wolfe sidesteps this entirely since every iterate is a convex combination of already-feasible points, never projected. The doc's proposed hard pairwise "decorrelation cap" constraint is deliberately NOT implemented as a separate constraint — Sigma's quadratic penalty already delivers the intended effect intrinsically (verified: two perfectly-correlated equal-mu ideas correctly get treated as "one bet," with a genuinely independent equal-vol third idea receiving MORE combined weight than the redundant pair) — adding a second, redundant hard cap would only matter in edge cases and would break the linear oracle's simple closed form. 15 tests in `StockSageAllocationOptimizerTests.swift`, covering an exact hand-derived single-idea case (box-cap-binds and heat-cap-binds variants), the de-correlation direction, box/heat-cap invariants across randomized inputs, and every degenerate-input guard. One test's first draft caught a genuine FIXTURE bug (not an algorithm bug) during verification: an "independent" comparison series shared a systematic drift term with the correlated pair, measuring an accidental 0.94 correlation instead of the intended ~0 — both the Python prototype and the real Swift code agreed exactly on the resulting allocation, which is what exposed the fixture as wrong rather than the code.
...
**2026-07-01 re-audit on #6 (still deferred, not a rejection):** Confirmed `StockSageAllocationOptimizer` does not exist anywhere in the 52-file StockSage tree, and the app has zero SwiftPM package dependencies (only `Accelerate` for vDSP dot-products) — a TRUE QP solver would need a new dependency, but the doc's own spec already scopes down to a hand-rollable projected-gradient loop, so that alone doesn't force a defer. What DOES force it: the spec's own signature is internally inconsistent. Its `ideas` tuple carries no spark/returns/vol field, yet the doc's prose says "Sigma from supplied rho + per-idea vol (derive from spark/returns)" — there is no volatility input to derive from, so Sigma's diagonal (and every off-diagonal rho·σᵢ·σⱼ term) cannot be constructed as specified. Separately, the stated objective "maximize w·mu − λ·w·Sigma·w" has no `lambda` parameter anywhere in the signature, unlike every OTHER magic constant here (heatCap/corrThreshold/maxIterations/convergenceTol all have defaults) — inventing one from nothing is a product/quant judgment call, not a mechanical translation. This would also be the FIRST iterative/non-closed-form numerical engine in a codebase where every other engine (including the already-shipped ranks 1-5) is closed-form and hand-verifiable — a materially higher verification bar for a doc-self-ranked "stretch," lowest-priority item that the doc itself says gives little benefit over already-shipped rank 1 ("ranks 1-5 already give most of the survival benefit far cheaper"). No code changes made. If ever revisited: re-spec first to add a returns/vol input to the `ideas` tuple and either add an explicit `lambda` or replace the mean-variance objective with something that doesn't need one.

**2026-06-28 re-audit + implementation:** #1 and #3 are ALREADY DONE — `StockSageCapitalAllocator.allocate(ideas:account:maxHeat:calibration:regime:)` (existing, current source) already merges both asks into ONE function: half-Kelly weighted (via `suggestedFraction`, the 20%-capped variant, not raw halfKelly), heat-capped via uniform proportional scaling, AND correlation-de-weighted via `StockSageCorrelationCluster.correlationAdjustedWeights` — plus goes beyond the backlog's ask with regime-adjusted sizing, crypto vol-targeting, the per-symbol vol-regime brake (EDGE_RESEARCH #1), and a net-cost gate. Uses the real `StockSageIdea` domain type rather than the backlog's proposed raw tuple signature — a cleaner API. #4 is substantially done: `StockSageRiskParity.targets()`/`vsEqualWeight()` + `StockSageRebalance.plan()` already exist with the exact shapes the backlog specifies, already composed directly in `MarketsView`/`MarketsRiskAllocationSection`, and the "correlation shock — keep a cash sleeve" caveat is already shown (unconditionally, which is more conservative than the doc's conditional ask). #2 (`suggestAdd`) was genuinely open — implemented + tested this session, composing `StockSageExpectedValue.ev`, `StockSageKelly.compute`, `StockSagePortfolioHeat.compute`, `StockSageClusterCheck.check`, `StockSagePositionSizer.size` exactly as specified (concentration gate checked FIRST, before edge). #5 (rebalanceToEdge, EV-weighted whole-book reweight) remains genuinely open — deferred, moderate scope. #6 (Sharpe QP optimizer) remains deferred as the doc itself frames it: "stretch," lowest priority, heaviest effort, only a local optimum.
