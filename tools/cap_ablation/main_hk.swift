...
// MARK: - Cap-vs-continuous weekly concentration ablation — EDGE-WEIGHTED FOLLOW-UP
//
// Fork of main.swift (2026-07-07 equal-risk run). ONE change from main.swift: the base
// weight rule is no longer flat w0 per fundable name — it is now the PLAN-pinned
// half-Kelly-shaped edge-proportional map, computed ONCE per week over the FULL fundable
// set F = {i : s_i>0}: w_i = 0.20 * s_i / max_{j in F}(s_j). CONTINUOUS feeds all of F;
// CAP-N feeds the ranked top-N prefix of F carrying the SAME w_i values — the cap is the
// only difference between arms, exactly as before. Everything else (panel load, TSMOM
// proxy, StockSageCorrelationCluster.correlationAdjustedWeights + StockSageDeflatedSharpe
// compiled VERBATIM, heat-scale port, turnover/cost accounting, block-t test) is
// byte-identical to main.swift.
//
// HYPOTHESIS (do not force it — a null increment CONFIRMS it): once CONTINUOUS sizes by
// edge instead of flat, it already captures the concentration benefit the prior run found,
// so the cap's advantage should shrink toward null. A significant cap win here would be a
// genuine finding to flag for owner.
...
while t + 5 <= lastIndex {
    ...
    // PLAN-pinned edge-weight map (THE ONE CHANGE from main.swift): computed ONCE per week
    // over the FULL fundable set F, w_i = 0.20 * s_i / max_{j in F}(s_j). CAP-N reuses these
    // same w_i for its top-N ranked prefix — no re-normalization within the shortlist.
    // F empty -> all-zero map (falls into the prior harness's empty-shortlist path below).
    // |F| == 1 -> max_j(s_j) == s_i -> w_i == 0.20 for that single name, both arms identical
    // that week (zero increment contribution — correct, not a bug, per the PLAN).
    var edgeWeight = [Double](repeating: 0, count: S)
    if let maxS = fundableIdx.map({ scores[$0] }).max(), maxS > 0 {
        for idx in fundableIdx { edgeWeight[idx] = 0.20 * scores[idx] / maxS }
    }
    ...
    for armIdx in 0..<armLabels.count {
        ...
        if !shortlist.isEmpty {
            ...
            let subWeights = shortlist.map { edgeWeight[$0] }   // THE ONE CHANGE from main.swift
