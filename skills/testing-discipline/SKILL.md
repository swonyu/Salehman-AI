...
**The WHIPPYX failure (2026-07-02, incident-ledger IL-23):** a symbol typo (`WHIPPYX` in the def vs `WHIPPYEX` in the history key) made `buildIdeas` return empty; a soft `guard let idea = ideas.first else { return }` passed silently and the test was reported green having verified NOTHING. Fixture/key typos read as "no data," not errors.
...
- **`git add` by name, never `git add -A`** — `PROJECT_CONTEXT.md` may be dirty from another concurrent session (never touch or stage it). (`tools/test_grok_bridge.py` is TRACKED since 713064e, 2026-07-02 — the old "keep it untracked" rule is obsolete; treat it like any other tracked file.)
...
- **Tests never quietly ratify an owner-gated decision.** Do not write a test that pins one side of a parked decision — a green assertion is not owner sign-off. The canonical owner-gate registry + BLOCKED procedure live ONLY in the `gated-scope` skill §1; check it before pinning anything decision-shaped.
...
- **Floating-point boundary literals:** a constant like 0.02 isn't exactly representable in binary64 — an exact-boundary test can fail despite correct math (bit `StockSageRebalanceTests.driftExactlyAtBandEdge`, logged 2026-06-28; incident-ledger IL-21). Pick bit-exact fixture values (powers-of-two-friendly, e.g. 0.25 with 2500/7500) or assert with a tolerance.
- **Two xcodebuilds on one DerivedData corrupt each other.** Concurrent session → isolated worktree + `-derivedDataPath <worktree>/.dd` (see `stocksage-mental-model` §7.5).
