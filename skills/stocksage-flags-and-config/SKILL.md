...
| `StockSageAdvisor.turnOfMonthEnabled` | `true` | `StockSageAdvisor.swift` (declaration doc-comment block) | ONLY `StockSageExpectedValue.seasonalityRankBonus` — a capped ±0.03 additive rank tilt in `rankByEV`/`bestOpportunity` (velocity surfaces exempt; NO advise()/sizing branch) | Shipped as a default-OFF status lock 2026-07-09, OWNER-ACTIVATED the same day ("WIRE ACTIVATE"), then **RATIFIED KEEP with the multi-year POWERED NULL in hand** (10 ETFs × 10y, DSR 0.831 — lane CLOSED; owner option "a": a deliberate disclosed preference, NOT an evidence promotion). Tilt hardening same day: direction-aware (buy +/sell −/neutral 0), |t|<1 noise-gated, in-progress-month excluded, top-15 untilted-EV prefetch, and DISCLOSED on every crowned surface (board line under EV sort + `tomTiltDisclosureSuffix` on best-opp/CTA/Best-now). State pinned by `StockSageTomGateTests` |
...
| `StockSageConvictionScaler` | Conviction-scaled per-trade risk fraction | **UPDATE 2026-07-09: no longer fully unwired** — `scaledRiskFraction` is now called display-only from `StockSageTodayPlan.rankedActions` (`StockSageTodayPlan.swift` ~L192-197) and rendered in `Views/MarketsTodayActionsCard.swift` (~L318/381) as an informational "conviction-scaled risk" line; the actual `shares`/`dollarsAtRisk` position-size math still uses the flat base risk fraction, so SIZING itself remains unwired — a backtest pass is still needed before this feeds real position size |
