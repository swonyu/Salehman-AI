...
- ~~**F10** — decimal-comma locale policy~~ ✅ **CLOSED 2026-07-04 (owner-approved "do it").** `StockSageInput.clean` is now grouping-aware: 3-digit groups → thousands, a lone comma + 1–2 trailing digits (no period) → the Saudi decimal ("2,5"→2.5), else nil. Fixes the silent 10× risk error; hand-derived truth table + 3 tests (falsifiability-probed); AUDIT §5 #3. No longer a gate. (F04 raw-`Double()` unification remains separately open.)
