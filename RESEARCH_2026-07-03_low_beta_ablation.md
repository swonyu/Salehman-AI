...
## UPDATE 2026-07-03 — multi-regime drawdown re-run (the residual this file named)
Re-ran the identical symmetric-beta + IVOL LOW-tercile-vs-BASE construction (plus downside-beta β⁻ and asymmetry β⁻−β⁺, covering the sibling downside-beta file too) on the **25-US-large-cap + ^GSPC 15y multi-regime panel** (`/tmp/multiregime_panel/panel.json`, 2011-07→2026-07, 3771 bars; reused, no fetch) — a window that actually CONTAINS the drawdowns the defensive premium is supposed to protect against. DSR from the compiled real `StockSageDeflatedSharpe`, trials=48 (32 full-period + 16 drawdown-only). Drawdown windows frozen from known S&P peak-to-trough dates: COVID_2020 (2020-02-19→04-30), BEAR_2022 (2022-01-03→10-13), SELLOFF_2018Q4 (2018-10-01→12-26).

**Full-period: verdict UNCHANGED — confirms the null.** No config clears DSR>0.95 (best full-period incremental 0.010); LOW−BASE remains significantly NEGATIVE in bull blocks at every horizon (t −2.2 to −4.6, p≤0.055), reproducing the original 5y sign/magnitude on 15y with real drawdowns included.

**Drawdown decomposition (the informative new part):** pooled across all 3 drawdowns the beta/IVOL LOW−BASE increment is POSITIVE at every horizon (+14 to +249bp/blk) but NEVER significant (n=5–18 blocks, p=0.39–0.85; best drawdown-only DSR 0.346, ivol H=126, n=5). Per episode:
- **COVID_2020 (fast liquidity crash):** strong, consistent protection — beta +145/+275/+101/+218bp, ivol +87/−30/+291/+419bp, db +150/+323/+106/+396bp. The defensive tilt worked here.
- **SELLOFF_2018Q4:** mostly positive protection (beta +155/+280/+342/+749bp).
- **BEAR_2022 (slow rate-hike bear):** NEGATIVE/flat — beta −34/−16/−17/−41bp, ivol −29/−20/−120/+18bp. The classic defensive tilt did NOT protect, plausibly because 2022 disproportionately hit richly-valued growth/tech names that carried historically low measured beta.
- **LOW_ASYM (β⁻−β⁺):** inconsistent/often NEGATIVE in drawdowns (−201bp 2018Q4@H21, −487bp@H126; only COVID positive) — the asymmetry construction is the noisier, less reliably protective leg, confirming the sibling downside-beta file's finding.

**Conclusion:** a real-drawdown window CONFIRMS the earlier null rather than overturning it. The mechanism the papers describe is VISIBLE in the data (real protection in the fast 2020 crash + 2018Q4) but is (a) statistically thin — only 2–3 drawdown episodes in 15y, (b) regime-selective — absent in the 2022 slow bear, and (c) net-of-cost, still doesn't clear the DSR bar full-period, because giving up return in the dominant bull stretches sinks every config. A nuanced NULL, not a flat one; not an activation candidate. Caveats: survivorship worse at 15y (today's survivors only); 25 mega-caps only; drawdown n genuinely underpowered; unlevered long-only leg (BAB's alpha is in the short leg + leverage, which this engine won't use). Repro: `/tmp/multiregime_drawdown_ablation/drawdown_beta.py` + compiled `main.swift`, reads `/tmp/multiregime_panel/panel.json`.
